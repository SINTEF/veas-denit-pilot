...
if [ "$#" -lt 2 ]; then
  ...
  echo "--seed Which controls if a seed will be passed to hydra.sweeper.sampler.seed. Necessary to draw different"
  echo " parameter combinations for jobs if using TPESampler. Does not work with GridSampler. Defaults to True"
  exit 1
fi
...
run_dir="\${paths.log_dir}/train/multiruns/${timestamp}_hopt-${CONFIG}"

# Loop to execute the Python script
for (( i=1; i<=NJOBS; i++ )); do
  # shellcheck disable=SC2089
  CMD="nohup python src/${TRAIN_SCRIPT} hparams_search=${CONFIG} ++hydra.sweeper.n_jobs=1 hydra.sweep.dir='${run_dir}${i}'"

  if [ "$SEED" -ne 0 ]; then
    CMD="${CMD} ++hydra.sweeper.sampler.seed=$((i + SEED))"
  fi
