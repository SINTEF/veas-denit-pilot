...
@pytest.mark.parametrize("only_relevant", [True, False])
def test_tsfresh(get_darts_example_dm, tmp_path, only_relevant):
    tsfresh_config = {
        "min_timeshift": 6,
        "max_timeshift": 6,
        "add_features_to": "future_covariates",
        "n_jobs": 0,
        "cache_path": os.path.join(tmp_path, "tsfresh.csv"),
        "include_target_features": True,
        "only_relevant_features": only_relevant,
    }

    dm = copy.deepcopy(get_darts_example_dm)
    dm.hparams["tsfresh"] = tsfresh_config
    original_future_covariates = copy.deepcopy(dm.hparams.data_variables["future_covariates"])
    if original_future_covariates is None:
        original_future_covariates = []

    assert dm.tsfresh_status is None

    dm.setup("fit")

    assert len(original_future_covariates) < len(dm.hparams.data_variables["future_covariates"])
    assert any("__" in var for var in dm.hparams.data_variables["future_covariates"])
    assert os.path.exists(os.path.join(tmp_path, "tsfresh.csv"))
    assert dm.tsfresh_status == "computed"

    dm2 = get_darts_example_dm
    dm2.hparams["tsfresh"] = tsfresh_config
    assert dm2.tsfresh_status is None

    dm2.setup("fit")

    assert dm2.tsfresh_status == "cache"
