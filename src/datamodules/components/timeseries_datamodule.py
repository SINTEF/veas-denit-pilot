...
_TSFRESH_REQUIRED_KEYS = ("min_timeshift", "max_timeshift", "add_features_to")
...
class TimeSeriesDataModule(LightningDataModule):
    ...
    def _finalize_setup(self, load_dir: Optional[str] = None):
        ...
        for dataset_name in self.data:
            ...
            if self.hparams.get("tsfresh"):
                assert all(key in self.hparams["tsfresh"] for key in _TSFRESH_REQUIRED_KEYS), (
                    f"The following keys are required to configure tsfresh: {_TSFRESH_REQUIRED_KEYS}, "
                    f"controlling minimum and maximum size of rolling window to compute features over, "
                    f"and where the features are used."
                )

                log.info("Performing tsfresh feature extraction!")

                cache_path = self.hparams.tsfresh.get("cache_path")

                # TODO: any other ways to verify if the cache file is still relevant?
                if os.path.exists(cache_path):
                    tsfresh_features = pd.read_csv(cache_path)
                    tsfresh_features = tsfresh_features.drop(columns=["Unnamed: 0"])

                    self.tsfresh_status = "cache"
                    log.info(f"Tsfresh features were loaded from file: {cache_path}")
                else:
                    if self.hparams.tsfresh.get("variables") is not None:
                        tsfresh_data = self.data[dataset_name][
                            self.hparams.tsfresh.get("variables")
                        ]
                    else:
                        tsfresh_data = self.data[dataset_name]

                    # tsfresh requires an entity_id used to group different timeseries. Here we assume one timeseries
                    tsfresh_data["tsfresh_entity"] = 0

                    tsfresh_data = self.set_dataset_precision(
                        tsfresh_data, self.hparams.get("precision", 32)
                    )

                    # TODO: filter out incompatible data, e.g. strings

                    data_rolled = tsf.utilities.dataframe_functions.roll_time_series(
                        tsfresh_data,
                        column_id=self.hparams.tsfresh.get("column_id", "tsfresh_entity"),
                        min_timeshift=self.hparams.tsfresh["min_timeshift"],
                        max_timeshift=self.hparams.tsfresh["max_timeshift"],
                        n_jobs=self.hparams.tsfresh.get("n_jobs", tsf.defaults.N_PROCESSES),
                    ).drop(columns=["sort", "tsfresh_entity"])

                    if (
                        self.hparams.tsfresh.get("only_relevant_features")
                        and not self.is_multivariate_target()
                    ):
                        y_aligned = data_rolled.groupby("id")[
                            self.hparams.data_variables["target"]
                        ].last()

                        # y must be a Series, but if data_variables["target"] is a list of length 1, we get a DataFrame
                        if isinstance(y_aligned, pd.DataFrame):
                            y_aligned = y_aligned.iloc[:, 0]

                        tsfresh_features = tsf.extract_relevant_features(
                            data_rolled,
                            y=y_aligned,
                            column_id="id",
                            n_jobs=self.hparams.tsfresh.get("n_jobs", tsf.defaults.N_PROCESSES),
                        ...
                        tsfresh_features = tsf.extract_features(
                            data_rolled,
                            column_id="id",
                        )

                    tsfresh_features = tsfresh_features.set_index(
                        tsfresh_features.index.map(lambda x: x[1]), drop=True
                    )

                    if cache_path is not None:
                        tsfresh_features.to_csv(cache_path)

                    self.tsfresh_status = "computed"

                tsfresh_features = tsfresh_features.set_index(
                    self.data[dataset_name].index[tsfresh_features.index]
                )
                # TODO: control impute / dropna behavior
                tsfresh_features = tsfresh_features.dropna(axis=1, how="any")

                if not self.hparams.tsfresh.get("include_target_features"):
                    tsfresh_features = tsfresh_features.drop(
                        columns=[
                            col
                            for col in tsfresh_features.columns
                            if any(
                                target in col for target in self.hparams.data_variables["target"]
                            )
                        ]
                    )

                # Remove the leading data of the first rolling window. TODO: control this?
                self.data[dataset_name] = self.data[dataset_name].iloc[-len(tsfresh_features) :]

                self.data[dataset_name] = pd.concat(
                    [self.data[dataset_name], tsfresh_features], axis=1
                )

                if isinstance(self.hparams.tsfresh["add_features_to"], str):
                    add_features_to = [self.hparams.tsfresh["add_features_to"]]
                else:
                    add_features_to = self.hparams.tsfresh["add_features_to"]

                for add_to in add_features_to:
                    if self.hparams.data_variables.get(add_to, None) is None:
                        self.hparams.data_variables[add_to] = tsfresh_features.columns.tolist()
                    else:
                        self.hparams.data_variables[add_to].extend(
                            tsfresh_features.columns.tolist()
                        )

                    log.info(
                        f"Constructed {len(tsfresh_features.columns)} features and added them to {add_to}!"
                    )

                all_data_variables.extend(tsfresh_features.columns.tolist())
            ...
            self.data[dataset_name] = self.set_dataset_precision(
                self.data[dataset_name], precision=self.hparams.get("precision", 32)
            )
    ...
    def call_function_on_univariate_series(
        ...
        **presenter_kwargs,
        ...
        :param transform: Whether to transform the univariate series before calling the function.
        ...
        for split_name in ["train", "val", "test"]:
            if (series_to_call == "all" or split_name in series_to_call) and self.has_split_data(
                ...
                res[split_name] = {}
                for series_type, serieses in self.get_split_data(
        ...
        **presenter_kwargs,
        ...
        for split_name in ["train", "val", "test"]:
            if (series_to_call == "all" or split_name in series_to_call) and self.has_split_data(
                ...
                split_data = self.get_split_data(split_name, transform=transform)
    ...
    def is_multivariate_target(self) -> Union[bool, None]:
        if self.hparams.get("data_variables", {}).get("target") is None:
            return None

        if isinstance(self.hparams.get("data_variables", {}).get("target"), str):
            return False
        else:
            return len(self.hparams.get("data_variables", {}).get("target")) > 1
    ...
    def _get_split_data_raw(
        ...
        split_datasets = [dataset for dataset in split_datasets if len(dataset) > 0]
