...
def run(
    ...
    # historical forecasts return the predictions. Conversely, backtest only returns metrics
    use_historical_forecasts = cfg.eval.get("plot") not in [None, False] or cfg.eval.get(
        "predictions"
    ) not in [None, False]
    ...
    metric_funcs, metric_names = src.metrics.utils.process_metric_funcs(
        ...
        inverse_transform_data_func=(
            None if use_historical_forecasts else inverse_transform_data_func
        ),
    )
    ...
    if (
        ...
        == "darts.models.forecasting.baselines.NaiveMean"
    ):
        model.__class__._supports_non_retrainable_historical_forecasts = property(
            lambda self: True
        )
    if use_historical_forecasts:
        ...
        for pred_i in range(len(predictions_seq)):
            ...
            if eval_kwargs.get("forecast_horizon", 1) == eval_kwargs.get("stride", 1):
                if (
                    ...
                    predictions_seq[pred_i] = [
                        darts.timeseries.concatenate(
                            predictions_seq[pred_i], ignore_time_axis=True
                        )
                    ]

                else:
                    predictions_seq[pred_i] = [
                        darts.timeseries.concatenate(predictions_seq[pred_i])
                    ]
        ...
        if (
            ...
            # take time index from prediction data seq
            for series_i in range(len(predictions_seq)):
                if is_multiple_series:
                    for pred_i in range(len(predictions_seq[series_i])):
                        start_time = cfg.eval.kwargs.start + pred_i
                        if is_multiple_series:
                            time_index = prediction_data_seq["series"][series_i][
                                start_time : start_time + len(predictions_seq[series_i][pred_i])
                            ].time_index
                        else:
                            time_index = prediction_data_seq["series"][
                                start_time : start_time + len(predictions_seq[series_i][pred_i])
                            ].time_index
                        predictions_seq[series_i][pred_i] = darts.TimeSeries.from_times_and_values(
                            time_index,
                            predictions_seq[series_i][pred_i].all_values(),
                        )
