...
def log_hyperparameters(object_dict: dict) -> None:
    ...
    cfg = object_dict["cfg"]
    model = object_dict.get("model", None)
...
def enable_powerset_resolver():
    from omegaconf import OmegaConf

    OmegaConf.register_new_resolver(
        "powerset",
        lambda x: ",".join(map(str, powerset(x, nonempty=True, container=list))),
        replace=True,
    )
...
def powerset(iterable, nonempty: bool = True, container: Type = tuple):
    "powerset([1,2,3]) --> () (1,) (2,) (3,) (1,2) (1,3) (2,3) (1,2,3)"
    s = list(iterable)
    res = itertools.chain.from_iterable(
        itertools.combinations(s, r) for r in range(1 if nonempty else 0, len(s) + 1)
    )
...
def optuna_filter_trials(
    storage: str, study_name: str, filter_func: Callable, new_study_name: Optional[str] = None
) -> None:
    """Helper function to filter trials from optuna study. If new_study_name is not set, the
    existing study will be overwritten with the filtered trials. If it is set, a new study with
    that name will be created.

    filter_func can be e.g.: lambda trial: trial.value > threshold

    :param storage: Storage uri of optuna study database. E.g. sqlite:///optuna.db if in same directory.
    :param study_name: Name of optuna study to filter.
    :param filter_func: Callable that takes a trial and returns True if trial should be filtered out.
    :param new_study_name: Name of new study to be created holding the filtered trials. If none, the original study
    will be overwritten with the filtered trials.
    ...
    filtered_trials = [trial for trial in old_study.get_trials() if not filter_func(trial)]
    ...
    try:
        old_study_direction = old_study.direction
    except KeyError:
        old_study_direction = "minimize"

    study = optuna.create_study(
        study_name=new_study_name,
        storage=storage,
        load_if_exists=True,
        direction=old_study_direction,
    )
    log.info("Adding trials to study, this may take a while!")
    study.add_trials(filtered_trials)
