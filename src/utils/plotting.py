...
def save_figure(fig, path, layout="constrained", figsize=None, despine=True):
    if despine:
        sns.despine(fig)
...
def set_matplotlib_attributes(font_size=9, font="DejaVu Sans", seaborn_style="ticks"):
    sns.set_theme(
        style=seaborn_style,
        rc={
            "font.size": font_size,
...
def set_figure_size(fig, column_span, height=None, base_double_cm=15.11):
    # latex uses 15.11078cm as textwidth for double column
    # \usepackage{layouts}
    ...
    if column_span == "single":
        fig_width = (base_double_cm * 0.48) * cm
    elif column_span == "double":
        fig_width = base_double_cm * cm
    elif isinstance(column_span, (int, float)):
        fig_width = column_span * cm
    else:
        raise ValueError()
    figsize = (fig_width, height * cm)
    print(f"set figsize to {figsize}")

    fig.set_size_inches(*figsize)


def get_covariate_plot_names():
    covariate_plot_names = {
        "ammonium": "Ammonium",
        "filterpressure_1": "Pressure bottom",
        "filterpressure_8": "Pressure top",
        "methanol": "Methanol",
        "nitrate_in": "Nitrate in",
        "nitrate_out": "Nitrate out",
        "orto-p": "Phosphorus",
        "oxygen": "Oxygen",
        "temp": "Temperature",
        "tunnelwater": "Water flow",
        "turb": "Turbidity",
    }
    return covariate_plot_names
...
def plot_prediction(
    ...
    if plot_weights:
        if not src.models.utils.is_rewts_model(model):
            ...
            nplots += 1
    fig, axs = create_figure(nplots, 1, sharex=True, figsize=(12, 9 * nplots))
    plot_i = 0
        ...
        if plot_data[data_type] is not None:
            if data_type == "series":
                for target_plot_i in range(
                    ...
                    series_to_plot.plot(label="actual", linewidth=0.75)
                    ...
                    for pred in prediction:
                        ...
                        if len(prediction) == 1:
                            pred.plot(label="prediction", linewidth=0.75, alpha=0.85)
                        else:
                            pred.plot(label="_nolegend_", linewidth=0.75, alpha=0.85)
                    plot_i += 1
            else:
                plt.sca(axs[plot_i])
                plot_data[data_type].plot(linewidth=0.75)
